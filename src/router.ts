...
const TASK_MODELS: Record<TaskType, { groq?: string; workers_ai?: string; opencode?: string }> = {
  summarize: { groq: "llama-3.1-8b-instant", workers_ai: "@cf/meta/llama-3.3-70b-instruct-fp8-fast", opencode: "nemotron-3-ultra-free" },
  validate: { groq: "llama-3.1-8b-instant", workers_ai: "@cf/meta/llama-3.3-70b-instruct-fp8-fast", opencode: "nemotron-3-ultra-free" },
  test: { groq: "llama-3.1-8b-instant", workers_ai: "@cf/meta/llama-3.3-70b-instruct-fp8-fast", opencode: "nemotron-3-ultra-free" },
  research: { groq: "llama-3.3-70b-versatile", workers_ai: "@cf/meta/llama-3.3-70b-instruct-fp8-fast", opencode: "nemotron-3-ultra-free" },
  design: { groq: "llama-3.3-70b-versatile", workers_ai: "@cf/meta/llama-3.3-70b-instruct-fp8-fast", opencode: "nemotron-3-ultra-free" },
  code_generation: { groq: "openai/gpt-oss-20b", workers_ai: "@cf/deepseek-ai/deepseek-r1-distill-qwen-32b", opencode: "nemotron-3-ultra-free" },
  ...
  judging: { groq: "llama-3.3-70b-versatile", workers_ai: "@cf/deepseek-ai/deepseek-r1-distill-qwen-32b", opencode: "nemotron-3-ultra-free" },
  architecture: { groq: "openai/gpt-oss-120b", workers_ai: "@cf/meta/llama-3.3-70b-instruct-fp8-fast", opencode: "nemotron-3-ultra-free" },
  ...
  // Now also has opencode fallback (2026-08-20, user on Zen) so reflection
  // doesn't stall on Workers AI quota either.
  reflect: { workers_ai: "@cf/meta/llama-3.3-70b-instruct-fp8-fast", opencode: "nemotron-3-ultra-free" },
};
...
export const DAILY_CAPS: Record<string, number> = {
  ...
  "workers_ai": 9500,
  // Zen free models (opencode.ai) — no published daily cap in the same
  // sense; they are free-tier but rate-limited per minute. Track as
  // request-count like Groq. 500/day is conservative headroom for an
  // autonomous cycle (~150 idea/critique/architecture calls).
  "opencode:nemotron-3-ultra-free": 500,
  "opencode:mimo-v2.5-free": 500,
  "opencode:big-pickle": 500,
};
...
async function tryWorkersAI(env: Env, model: string, req: InferenceRequest): Promise<string | null> {
  ...
  try {
    ...
    // 4006 daily free allocation exhausted (and any other AI.run throw)
    // must fall through to the next tier (Zen) instead of bubbling as
    // "Inference exhausted" — found live 2026-08-20 on architecture-phase
    // event_a0cbe: Workers AI threw 4006, never reached Zen, queue stalled.
    return null;
  }
}

async function tryOpencode(env: Env, model: string, req: InferenceRequest): Promise<string | null> {
  if (!env.OPENCODE_API_KEY) return null;
  const capKey = `opencode:${model}`;
  const used = await unitsUsedToday(env, "opencode", model);
  if (used >= (DAILY_CAPS[capKey] ?? DAILY_CAPS["opencode:nemotron-3-ultra-free"] ?? 500)) return null;
  ...
  try {
    ...
    const data: any = await res.json();
    // Zen is request-count capped like Groq, not neuron-capped.
    await recordUsage(env, "opencode", model, req.task_type, 1);
    const content = data.choices?.[0]?.message?.content;
    return typeof content === "string" ? content : null;
  } catch {
    return null;
...
/**
 * Routes an inference request through Groq -> Workers AI -> Zen, per spec §6
 * plus 2026-08-20 Zen addition (user on Zen, free tier). Returns null
 * rather than throwing if all are exhausted or fail — caller decides whether
 * to queue.
 */
export async function routeInference(env: Env, req: InferenceRequest): Promise<{ text: string; provider: string; model: string } | null> {
  ...
  const tryOrder: Array<"groq" | "workers_ai" | "opencode"> = req.pinned_provider
    ? [req.pinned_provider as "groq" | "workers_ai" | "opencode"]
    : ["groq", "workers_ai", "opencode"];
  ...
  for (const provider of tryOrder) {
    ...
    const text =
      provider === "groq"
        ? await tryGroq(env, model, req)
        : provider === "workers_ai"
          ? await tryWorkersAI(env, model, req)
          : await tryOpencode(env, model, req);
